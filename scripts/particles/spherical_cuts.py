...
def spherical_cut_plot():
    ...
    # These are precomputed r and z values for the above values of sigma and the paper model parameters
    r_values = np.array([
    ...
    use_idxs = [4, 16, 28]
    npa_sigmas = npa_sigmas[use_idxs]
    z_values = z_values[use_idxs]
    r_values = r_values[use_idxs]
    ...
    def _plot_sphere_slice_border(r: float, h: float, c: Union[str, np.ndarray], alpha: float = 1.):
        angle = np.arccos(h / r)
    ...
    def _plot_sphere_slice_caps(r: float, h: float, c: Union[str, np.ndarray], alpha: float = 1.):
        angle = np.arccos(h / r)
    ...
    def _plot_sphere_slice(r: float, h: float, c: Union[str, np.ndarray], alpha: float = 1.):
        _plot_sphere_slice_border(r, h, c, alpha)
        _plot_sphere_slice_caps(r, h, c, alpha)

    def _plot_trajectory(X: np.ndarray, alpha: float = 1., c=None):
        x, y, z = X.T
    ...
    def _plot_arrow(origin=(0, 0, 0), dest=(1, 0, 0), c=None):
        x1, y1, z1 = origin
        x2, y2, z2 = dest
    ...
    for i, npas in enumerate(npa_sigmas):
        ...
        _plot_sphere_slice(r_values[i, 0], z_values[i, 0], colour, alphas[i])
