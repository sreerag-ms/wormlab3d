...
class Midline3DFinder:
    ...
    def _init_convergence_detector(self) -> ConvergenceDetector:
        ...
        cd = ConvergenceDetector(
            shape=(2 + p.depth - p.depth_min,),  # +1 for loss_global, +1 for loss_head_and_tail, rest for losses_depths
    ...
    def _calculate_head_and_tail_losses(self, points_2d: List[torch.Tensor]) -> torch.Tensor:
        ...
        # Use the highest resolution points (depth 0)
        if len(points_2d) == 0:
            return total_loss
            
        pred_points = points_2d[0]  # Shape: (batch, N, 2)
        batch_size = pred_points.shape[0]
        
        for batch_idx in range(batch_size):
            # For each camera view (frame_id: 0, 1, 2)
            for frame_id in range(3):
                # Find matching coordinates in dataset (no trial_id filtering needed as all data is for this trial)
