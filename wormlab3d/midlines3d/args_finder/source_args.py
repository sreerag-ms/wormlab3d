...
class SourceArgs(BaseArgs):
    def __init__(
        ...
        # Load head and tail coordinates CSV data if flag is enabled
    ...
    def _load_head_tail_coordinates(self, csv_path: str, trial_id: int):
        """
        Load head and tail coordinates CSV data and filter for specified frames.
        
        Args:
            csv_path: Path to the CSV file
            trial_id: Trial ID (for logging purposes)
            
        Returns:
            pandas.DataFrame or None: Filtered dataframe with coordinate data, or None if loading failed
        """
        if not csv_path:
